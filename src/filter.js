...
var grp_filter = function() {
	...
	function init() {
		...
		if (_excludes.length || _highlights.length) {
			document.body.addEventListener("DOMNodeInserted", filterEntries,
					false);
		}
	}
	...
	function filterEntries(e) {
		...
		if (_preferHighlights) {
			if (_highlights.length) {
				b = checkEntry(minifiedTitle, element, _rxHighlights,
						"entry-highlighted");
			}
			if (!b && _excludes.length) {
				b = checkEntry(minifiedTitle, element, _rxExcludes,
						"entry-filtered");
				if (_hideExcluds && b)
					element.parentNode.removeChild(element);// element.style.display="none";
			}
			...
			if (!b && _highlights.length) {
				checkEntry(minifiedTitle, element, _rxHighlights,
						"entry-highlighted");
			}
		}
	...
	function minifyTitle(title) {
		return trim(title.replace(_minifyRx, " ").replace(/ +/g, " ")
				.toLowerCase());
	}
